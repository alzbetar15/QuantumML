...
def data_embedding(X): #encode input X with 8 qubits with amplitude encoding
    ...
    args: X - the data
    '''
    AmplitudeEmbedding(X, wires=range(9), normalize=True)
...
def Benchmarking(dataset, Unitaries, U_num_params, filename, circuit, steps, snr, f1, f2, binary=True):
    '''
    ...
    for i in range(I):
        ...
        f.write("Loss History for " + circuit + " circuits, " + str(f1) + str(f2) + " Amplitude with " +'cross entropy' + ' trained with: ' + lend + ' with snr: ' +str(snr))
        f.write("\n")
