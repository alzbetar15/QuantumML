...
def quantum_data():
    ...
    for i in range(0,101):
        ...
        emoji_style = {
            "displaytext": {
                ...
                #"y": "😎",
                #"z": "❤️",
                'statevector': "$\bra{\psi} input state$"
                #
            },
        }
    ...
    #plt.savefig('Quantum_data1/new run '+str(i)+'output waveform '+str(p)+'.png')
        ...
        circ.draw('mpl')
        circuit_drawer(circ, output='mpl', plot_barriers=True)
        circ.draw("mpl", cregbundle=False, initial_state=True, reverse_bits=True,style=emoji_style)
        #circ.decompose(reps=2).draw('mpl')
        #plt.savefig(str(i)+'circuit other method E 11'+str(p)+'.png')
            #break
        plt.savefig('stitch_'+str(i)+'.png')
        #plt.savefig('test2_'+str(i)+'.png')
        plt.close()
